<template>
  <div style="height: 100%" class="zl-sysUser" v-loading="loading">
    <div class="header">
      用户昵称:
    ...
    <el-dialog :title="title" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
      <el-input placeholder="请输入用户名" v-model="UserName" clearable :disabled="disabled"></el-input>
      <el-input placeholder="请输入昵称" v-model="NickName" clearable></el-input>
      <el-input placeholder="密码" v-model="UserPwd"  clearable :disabled="disabled"></el-input>
  ...
  methods: {
    ...
    confirm() {
      ...
      if (!(this.insertId == false)) {
        ...
        arr.push(obj);
        axios
          .get("/api/User/UpdateAirCityInfo", {
            params: {
              updateUserInfo: obj
            }
          })
      ...
      axios
        ...
        .then(function(response) {
          ...
          data = data.map(item => {
            ...
            item.CreateTime = CreateTime.split('T')[0];
            return item;
          });
